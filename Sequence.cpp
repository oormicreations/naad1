...
CSequence::~CSequence(void)
{
	if((m_BolIdSeq)||(m_BolStatusSeq)) DeleteBolSequence();
}
...
bool CSequence::CreateBolSequence(CLoop loop)
{
	...
	if((m_BolIdSeq)||(m_BolStatusSeq)||(m_BolVarSeq)) DeleteBolSequence();
	...
	if(!(allocbolid && allocbolstatus && allocbolvar)) return false;
